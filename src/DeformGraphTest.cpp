...
int main() {
    ...
    const char* src_filename = "./data/src.obj";
    const char* dst_filename = "./data/tar.obj";

    TriMesh mesh_in(src_filename);
    TriMesh mesh_out(dst_filename);

    double searchRadius = 0.3;
    double angleThreshold = 90.0;
    double dilationRadius = 0.2;
    std::vector<int> boundaryIndicator;
    boundary_estimate(mesh_out, boundaryIndicator, searchRadius, angleThreshold, dilationRadius);

    std::vector<int> sampling_index;
	double sampling_dist = 0.3;
	...
	do {
		...
		for (size_t i=0; i<graph.node_pos.size(); ++i) {
            ...
            if (boundaryIndicator[indices_node[0][0]] != 0) continue; // ignore boundary
