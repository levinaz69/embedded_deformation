...
void boundary_estimate(const TriMesh& mesh, std::vector<int>& boundaryIndicator, double searchRadius, double angleThreshold, double dilationRadius)
{
    ...
    pcl::PointCloud<PointType>::Ptr cloudData = _trimesh2pcl_data(mesh);
    Boundaries::Ptr boundaries;
    ...
    boundaryIndicator.resize(boundaries->size());
    for (int i = 0; i < (*boundaries).size(); i++)
    {
        boundaryIndicator[i] = boundaries->points[i].boundary_point != 0;
    }
